...
const Champ = ({teamDB, matchDB}) => {
    ...
    matchDB.forEach(match => {
        ...
        const winComp = teamDB.filter(team => team.matchID == matchId && team.teamName == winner)[0]
        const loseComp = teamDB.filter(team => team.matchID == matchId && team.teamName == loser)[0]
    ...
    const sortByInput = () => {
        ...
        if(userInput){
            const filterList = ChampWinPercentage.filter(champ => champ.champ.toLowerCase().startsWith(userInput));
    ...
    return(
        <>
            <input type="search" ref={searchRef} onChange={() => sortByInput()} style={{marginBottom: '30px'}}/>
            <table style={{textAlign: 'left'}}>
                <thead style={{}}>
                    <th style={{ width: '30ch', paddingBottom: '50px'}}>Name</th>
